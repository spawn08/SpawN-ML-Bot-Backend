...
def train(model_name):
    ...
    # f.close()
    output = list((data.get('rasa_nlu_data').get('common_examples')))
    print(output)
    ...
    for pattern in output:
        ...
        words.extend(w)
        documents.append((w, pattern['intent']))
    ...
    model.fit(train_x, train_y, n_epoch=130, batch_size=8, show_metric=True)
